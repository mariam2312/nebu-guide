...
class _InfoBankScreenState extends State<RestrictionsScreen> {
                      ...
                      onChanged: (bool? checked) {
                        setState(() {
                          ...
                          if (selectTodaysRestrictions) {
                            ...
                            selectVip = false;
                            selectPro = false;
                            selectBasic = false;         selectAll=false;
                          }
                        });
                      },
                    ),
                      ...
                      onChanged: (bool? checked) {
                        setState(() {
                          ...
                          if (selectBasic) {
                            ...
                            selectMonthRestrictions = false;
                            selectVip = false;
                            selectPro = false;         selectAll=false;
                          }
                        });
                      },
                    ),
                  ...
                  onPressed: () {
                    ...
                    if (selectedRoles.isEmpty &&
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Please select at least one role or restriction.'),
  ...
  Future<void> generatePdf() async {
    ...
    List<pw.Widget> createContent(List<Restriction> restrictions, String title) {
      return [
            ...
            for (var restriction in restrictions)
              pw.TableRow(
                children: [
                  ...
                  pw.Container(
                    height: 30,
                    child: pw.Center(child: pw.Text("${restriction.Title}",style: pw.TextStyle(font: ttf, fontSize: 12), textDirection: pw.TextDirection.rtl))),
    ...
    if (selectedRoles.contains('ALL')) {
      ...
      pdf.addPage(
        pw.MultiPage(
          build: (pw.Context context) => createContent(allRestrictions, "All Restrictions"),
        ),
      );
    } else {

      if (selectedRoles.contains('TODAY')) {
        final todayRestrictions = guideProvider.restrictionsList.where((r) => isTodaysRestriction(r)).toList();
        pdf.addPage(
          pw.MultiPage(
            build: (pw.Context context) => createContent(todayRestrictions, "Today's Restrictions"),
          ),
        );
      }
      if (selectedRoles.contains('MONTH')) {
        final monthRestrictions = guideProvider.restrictionsList.where((r) => isCurrentMonthRestriction(r)).toList();
        pdf.addPage(
          pw.MultiPage(
            build: (pw.Context context) => createContent(monthRestrictions, "Month's Restrictions"),
          ),
        );
      }
      if (selectedRoles.contains('VIP')) {
        final vipRestrictions = guideProvider.restrictionsList.where((r) => r.Allowed_Plans?.contains('vip') ?? false).toList();
        pdf.addPage(
          pw.MultiPage(
            build: (pw.Context context) => createContent(vipRestrictions, "VIP Restrictions"),
          ),
        );
      }
      if (selectedRoles.contains('PRO')) {
        final proRestrictions = guideProvider.restrictionsList.where((r) => r.Allowed_Plans?.contains('pro') ?? false).toList();
        pdf.addPage(
          pw.MultiPage(
            build: (pw.Context context) => createContent(proRestrictions, "PRO Restrictions"),
          ),
        );
      }
      if (selectedRoles.contains('BASIC')) {
        final basicRestrictions = guideProvider.restrictionsList.where((r) => r.Allowed_Plans?.contains('basic') ?? false).toList();
        pdf.addPage(
          pw.MultiPage(
            build: (pw.Context context) => createContent(basicRestrictions, "Basic Restrictions"),
          ),
        );
      }
      if (selectedRoles.isNotEmpty) {
        ...
        for (var role in selectedRoles) {
          if (['VIP', 'PRO', 'BASIC', 'TODAY', 'MONTH', 'ALL'].contains(role)) {
          ...
          pdf.addPage(
            pw.MultiPage(
                build: (pw.Context context) => createContent(roleRestrictions, "Restrictions for $role")
            ),
          );
                ...
                Expanded(
                    child:  ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        Row(
                        children: [
                              Text("${restriction.Title!.split(' ').take(3).join(' ')}"),
                          Text("(${index+1})"),])
                            ],
                          );
                      },
